...
{
    {
        if (m_message_rate_timer.hasPassed(1000 / m_message_rate))
        {
            request_messages(MAVLINK_MSG_ID_ATTITUDE);
        }
...
void MavBridge::request_messages(uint32_t msg_id)
{
    MavMsg m_mav_msg;
    m_mav_msg.system_id = m_system_id;
    m_mav_msg.component_id = m_component_id;
