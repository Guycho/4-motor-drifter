...
void MavBridge::init(const MavBridgeConfig &config) {
    ...
    m_message_rate_timeout_level_1 = 1e4 / config.message_rate_level_1;
    m_message_rate_timeout_level_2 = 1e4 / config.message_rate_level_2;
...
void MavBridge::handle_sessage_requests() {
    if (m_message_rate_timer_level_1.hasPassed(m_message_rate_timeout_level_1)) {
        ...
        m_message_rate_timer_level_1.restart();
    }
    if (m_message_rate_timer_level_2.hasPassed(m_message_rate_timeout_level_2)) {
        for (uint8_t i = 0; i < Config::MavlinkBridge::num_messages_per_group; i++) {
            if (m_message_group_level_2[i] != UINT32_MAX) {
                request_message(m_message_group_level_2[i]);
            }
        }
        m_message_rate_timer_level_2.restart();
    }
}
